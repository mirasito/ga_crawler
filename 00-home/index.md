...
- **[[Текущие приоритеты — Phase 8 planned, execute next]]** — что делать прямо сейчас (**Phase 8 planned 2026-05-13** — 5 plans в 4 waves; W0 spike требует operator (~60-90 min), waves 1-3 autonomous (~30 min); next command `/gsd-execute-phase 8`)
...
- **Решения v1.1 (живые, 2026-05-13):** **[[Goldapple PDP renders volume в structured flexbox blok, не в microdata]]** (PARSE-FIX-01 — selectolax 0.4 Lexbor `:lexbor-contains` primitive; 88/88 SKU run #13 had NULL volume_norm) · **[[viled volume — в attributes Размер JSON-поле, не в name]]** (PARSE-FIX-03 — `props.pageProps.attributes[].name == "Размер"`; clothing fixture confirmed, beauty PDP needs 15-min Wave-0 probe) · **[[viled Размер JSON path — nested attributes 0 attributes, не item attributes]]** (RES-08-01 — Phase 8 research empirical trace; load-bearing correction of STACK.md tentative path) · **[[Yandex Cloud kz1 over Hetzner EU для KZ-IP anti-bot]]** (DEPLOY-01..06 — user choice; ~₸6800/мес vs €4.50 Hetzner; Camoufox×Yandex compat MEDIUM-risk, требует pre-cron smoke) · **[[Forward-only no backfill runs 1-13]]** (HTML gone, matcher idempotent выдаст те же 0 matches, auto-suggest 4-week median rolls naturally к run #17)
