...
- **[[Текущие приоритеты — sub-bucketing v3 закрыт top-10 honest 4312 matches]]** — что делать прямо сейчас (**run-21 финал: viled 6 359 / GA 3 744 / matches 4 312 @ 81.74% honest rate**; viled multi-variant per-variant capture + perfume concentration sub-bucketing + body-part qualifier + default-face heuristic закрыли все user-reported FP-классы; 583/583 unit tests; xlsx прилетел message_id=42; 8 коммитов запушены `04060d5..dfa4ab7`)
- 5337-matches trail: ~~[[Текущие приоритеты — 5337 matches достигнуто, локальный режим продолжается]]~~ — superseded 2026-05-16 evening forensic review (forensic review показал что 5337 inflated cross-class FPs; правильный count 4312 honest после sub-bucketing)
...
- Phase 9 contexted trail: ~~[[Текущие приоритеты — Phase 9 contexted, plan next]]~~ — superseded 2026-05-14 evening (4 PLAN.md ready; plan-checker PASS 8/8; execute next)
- Phase 8 close trail: ~~[[Текущие приоритеты — Phase 8 W1 done, W2 next]]~~ — superseded 2026-05-14 (Phase 8 закрыт через Plans 08-03/08-05 doc cascade 2026-05-13; 5/5 PARSE-FIX reqs Complete; Phase 9 contexted сейчас)
...
- **Решения v1.1 (живые, 2026-05-13..05-14):** **[[Goldapple PDP renders volume в structured flexbox blok, не в microdata]]** (PARSE-FIX-01 — selectolax 0.4 Lexbor `:lexbor-contains` primitive; 88/88 SKU run #13 had NULL volume_norm) · **[[Goldapple brand+name extraction — h1 spans CSS class substring, не itemprop microdata]]** (SPIKE-08-01 — W0 spike 2026-05-14 invalidated Plan 08-03 microdata premise; 0/30 PDPs have `<meta itemprop="name">` at product level; pivot к `h1 [class*="_ga-pdp-title__brand_"] + [class*="_ga-pdp-title__name_"]` substring CSS match; explains v1.0 cross-product contamination) · **[[viled volume — в attributes Размер JSON-поле, не в name]]** (PARSE-FIX-03 — `props.pageProps.attributes[].name == "Размер"`; clothing fixture confirmed, beauty PDP confirmed via W0 viled-contre-jour-408872 fetch) · **[[viled Размер JSON path — nested attributes 0 attributes, не item attributes]]** (RES-08-01 — Phase 8 research empirical trace; load-bearing correction of STACK.md tentative path) · **[[Yandex Cloud kz1 over Hetzner EU для KZ-IP anti-bot]]** (DEPLOY-01..06 — user choice; ~₸6800/мес vs €4.50 Hetzner; Camoufox×Yandex compat MEDIUM-risk, требует pre-cron smoke) · **[[Forward-only no backfill runs 1-13]]** (HTML gone, matcher idempotent выдаст те же 0 matches, auto-suggest 4-week median rolls naturally к run #17)
- **Решения Phase 9 (живые, 2026-05-14, locked в CONTEXT.md):** **[[Pydantic RawProduct validation at SqliteSnapshotWriter — 5 percent reject-rate gate orthogonal to PARSE-FIX-04]]** (D-903 — defense-in-depth cascade; schema rejected >5% → run failed `schema_validation_rejected_rate`; ORTOGONAL к Phase 8 PARSE-FIX-04 50% null-rate gate) · **[[Per-retailer Pydantic schema split — goldapple strict viled relaxed для legitimate Nones]]** (D-904 — `GoldappleRawProduct` strict, `ViledRawProduct.volume_raw: NonEmptyStr | None` для Contre-Jour-style legitimate Nones; rejected unified strict) · **[[pytest -m live — operator-only opt-in документируется в README §8]]** (D-905 — manual operator runbook; weekly-run.sh не меняется; drift → `.planning/research/parser-drift-YYYY-MM-DD.md`; deferred auto-cron к v1.2) · **[[P2 bundle gate — time-budget 8h elapsed W0+W1 для GO NO-GO checkpoint]]** (D-902 — TEST-HARNESS-04/05 conditional via git commit timestamp arithmetic; rejected test-count delta proxy; user override allowed)
