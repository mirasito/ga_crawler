...
- **D-305:** **Bilingual slug-fy + longest-prefix-in-whitelist match**. Slug-fier из каждой alias-строки производит ДВА варианта slug: ASCII (после NFKD + accent strip + transliterate Cyrillic→Latin) И Cyrillic-preserved. Алгоритм: lowercase → non-alphanum → `-` → collapse multi-`-`. **Lookup mechanism (revised in Wave 7 gap-closure 03-08, was: `exact match against product-slug dict`):** sitemap parser строит `brand_bucket: dict[brand_token, list[url]]` через `index_by_brand_token(slug_map, known_brand_tokens)`. Для каждого product-slug алгоритм выбирает **самый длинный** prefix (depth 1..3), который ∈ `known_brand_tokens` (whitelist, precomputed orchestrator-side из `slug_fy_bilingual` всех viled brand aliases) — URL уходит в bucket этого longest-prefix И ТОЛЬКО туда; `intersect_brand_pool` затем делает `bucket.get(brand_slug)` — exact-match-by-key, не substring/prefix-by-string. Pitfall 3 защита от substring-contamination обеспечивается **структурно**: `tom-ford-beauty-eye-cream` URL никогда не попадёт в `bucket['tom-ford']` если `'tom-ford-beauty'` ∈ whitelist (longest-prefix match выберет более конкретный bucket первым). **Operator opt-in disambiguation:** если оператор включил в `viled_brands` только `Tom Ford` (без `Tom Ford Beauty`), `tom-ford-beauty-*` URL'ы попадут в `bucket['tom-ford']` (default fallback) — это намеренное поведение, а не нарушение D-305: brand-extension семья classification — operator-driven через viled_brands list, а не automatic. Если нужно строгое исключение beauty-line — добавить `Tom Ford Beauty` отдельным брендом в viled-snapshot. Rapidfuzz/fuzzy откладывается в v2 (REQ MATCH-V2-01).
