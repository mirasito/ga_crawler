...
| 6. Telegram Delivery + Ops/Business Split | 2/6 | In progress (Wave 0 setup shipped 2026-05-12 Plan 06-01: aiogram 3.28.2 + [tool.ga_crawler.deliver] 6-key namespace D-614 + .env.example D-612 + 3 conftest fixtures + 5-section golden file + 10 skip-marked stubs as red→green targets for Waves 1-4; Wave 1 foundations shipped 2026-05-12 Plan 06-02: delivery/{__init__,stats,config,message_builder}.py pure-Python modules with D-607 8-key namespace + D-611/D-614 config dataclasses + D-610 build_ops_alert single template with html.escape Pitfall A + Asia/Almaty %z numeric offset W1 fix + business_caption D-514 helper; 5 golden-file scenarios populated with real outputs; 50 new GREEN tests close 4 Wave-0 stubs + 1 Wave-0 canary regression added; 626 → 677 passed, 11 → 7 skipped; structural canaries enforce no-load_dotenv in config + no-aiogram-imports in pure-Python wave) | - |
