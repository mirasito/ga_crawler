...
| 6. Telegram Delivery + Ops/Business Split | 4/6 | In progress (Wave 0 + Wave 1 + Wave 2 shipped earlier 2026-05-12; Wave 3 orchestrator + CLI shipped 2026-05-12 Plan 06-04: runners/delivery_run.py 7-step sync pipeline composing config + gate + message_builder + telegram_client + stats via asyncio.run(_send_async) under async with bot per D-602 Pitfall B; D-605 (Telegram failure ≠ run failure — runs.status untouched + xlsx persists), D-606 (6-enum delivery_status), D-607 (Pitfall 6 single patch_stats — exactly 2 static call sites enforced by structural canary), D-608 (idempotency + --force + --dry-run + exit codes 0/2/3), D-611 (asymmetric ENV) all enforced; Pitfall C defense-in-depth xlsx path containment; Pitfall D message_id sentinel semantics (non-split caption_id==document_id; split distinct); cli.py deliver-run subcommand with 6 flags + _cmd_deliver handler (load_dotenv ONLY here per RESEARCH caveat #4); 27 new GREEN tests across 2 closed Wave-0 stubs (19 delivery_run + 8 cli_deliver); 707 → 735 passed, 5 → 3 skipped; 5 Rule-1/2/3 auto-fixes inline (Python 3.12 sqlite ISO-string coercion; capsys instead of caplog; rfind for double-JSON dry-run; pending+dry_run → exit 0; WAVE3_CLOSURES canary)) | - |
