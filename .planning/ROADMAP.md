...
- [x] 05-01-PLAN.md - Wave 0: foundation — pyproject `[tool.ga_crawler.report]` namespace (D-516) + `reporter/__init__.py` + `config.py` (ReportConfig mirror MatchConfig) + `stats.py` (REPORT_STATS_KEYS 7-tuple D-514 + ReportStatsBuilder 4-way disjoint); pandas+xlsxwriter+openpyxl+tzdata deps; synthetic_report_run + tmp_reports_dir + openpyxl_workbook_reader conftest fixtures; D-504 golden summary fixture **[Complete 2026-05-11; 23 tests added; 0 deviations; SUMMARY: 05-01-SUMMARY.md]**
- [x] 05-02-PLAN.md - Wave 1: pure builders — reporter/queries.py 5 SQL constants (PER_SKU JOIN-back per Pitfall 9 + NOT EXISTS per Pitfall 8 + SQL discount per Pitfall 10) + reporter/excel_builder.py (D-503 Russian headers + D-505 3-color CF mid_value=0 anchor + D-506 always-4-sheets + D-508 CF-2-sheets-only + Pitfall 1 engine=xlsxwriter explicit + T-05-injection sanitization) + reporter/summary_builder.py (D-504 template source-locked + zero-match Top-3 omission) **[Complete 2026-05-11; 49 tests added (14 queries + 23 excel_builder + 12 summary_builder); 0 deviations; SUMMARY: 05-02-SUMMARY.md]**
- [x] 05-03-PLAN.md - Wave 2: filesystem service — reporter/archive.py (D-512 ISO-week derivation with Pitfall 4 year-boundary 2027-01-01→2026-W53 + D-510 atomic write `*.xlsx.tmp`+os.replace + D-515 check_size_guard flag-only never-raises); reports/.gitkeep + .gitignore exclude reports/*.xlsx + reports/*.xlsx.tmp **[Complete 2026-05-12; 35 tests added (5 smoke + 10 iso-week + 12 atomic-write + 8 size-guard integration); 0 deviations; SUMMARY: 05-03-SUMMARY.md]**
- [x] 05-04-PLAN.md - Wave 3: orchestrator — runners/reporter_run.py 7-step sync mirror matcher_run + D-507 status-gate REUSE read_run_status from matcher.strict_key + path-traversal containment + Pitfall 6 single patch_stats + D-515 flag-only never-fails-run **[Complete 2026-05-12; 15 integration tests added covering D-507 (3 skip cases) + 4-sheet success path + Russian headers + CF/freeze/autofilter + D-510 idempotency + D-514 7-key stats namespace + D-515 flag-only + D-405 KPI verbatim + T-05-injection e2e + DATA-05 exception-propagation + defensive NULL started_at; 0 deviations; SUMMARY: 05-04-SUMMARY.md]**
- [x] 05-05-PLAN.md - Wave 4: composition — main_run.py reporter step post-matcher with explicit m_result.status=='success' gate + MainRunResult extended 4 fields per D-514 + DATA-05 reporter-exception path; cli.py D-509 report-run --run-id N subcommand mirror matcher-run (type=int + --output-dir via dataclasses.replace) **[Complete 2026-05-12; 16 integration tests added (8 main_run composition + 8 CLI subprocess); 1 Rule 1 deviation (Unicode stdout fix on Windows via sys.stdout.buffer.write); SUMMARY: 05-05-SUMMARY.md]**
- [x] 05-06-PLAN.md - Wave 5: doc cascade — REQUIREMENTS.md REPORT-01..06 closed + REPORT-01 amendment per D-502 + STATE.md D-514/D-515/D-405 cascade + Plan Execution Metrics + ROADMAP.md Phase 5 plan list + Progress 6/6 **[Complete 2026-05-12; 0 tests added (docs-only); 0 deviations; 3 files modified (REQUIREMENTS.md + STATE.md + ROADMAP.md); SUMMARY: 05-06-SUMMARY.md]**
**UI hint**: no
...
- [x] 06-01-PLAN.md - Wave 0: pyproject [tool.ga_crawler.deliver] namespace (D-614) + aiogram>=3.27,<4.0 dep + .env.example (D-612) + .gitignore audit + 3 new tests/conftest.py fixtures + 10 skip-marked test stubs + golden-file scaffold **[Complete 2026-05-12; aiogram 3.28.2 resolved; 16 new green canaries + 10 new skip-marked stubs; 626 passed, 11 skipped; 0 deviations; SUMMARY: 06-01-SUMMARY.md]**
- [x] 06-02-PLAN.md - Wave 1: delivery/__init__.py + config.py (DeliverConfig + DeliverEnvConfig D-611/D-614/RESEARCH caveat #4) + stats.py (DELIVER_STATS_KEYS 8-tuple D-607 + 5-way disjoint) + message_builder.py (build_ops_alert D-609/D-610 + Pitfall A html.escape + Pitfall E Asia/Almaty tz) **[Complete 2026-05-12; 50 new GREEN tests across 4 closed Wave-0 stubs (14 stats + 12 config + 22 message_builder + 2 five-way); 5-scenario golden file populated with real outputs (Almaty `+0500` numeric offset W1 fix); structural no-load_dotenv + no-aiogram canaries pass; 626 → 677 passed, 11 → 7 skipped; 1 Rule 3 deviation auto-fixed (Wave-0 stub-inventory canary narrowed 10→6 + WAVE1_CLOSURES regression added); SUMMARY: 06-02-SUMMARY.md]**
- [x] 06-03-PLAN.md - Wave 2: delivery/gate.py (evaluate_gate D-604 4-check first-fail-wins REUSES matcher.strict_key.read_run_status) + delivery/telegram_client.py (open_bot + send_*_with_policy + tenacity wait_chain(5,15,45) per RESEARCH caveat #2 + Pitfall A fail-fast exclusions) **[Complete 2026-05-12; 29 new GREEN tests across 2 closed Wave-0 stubs (10 gate + 19 telegram_client); aiogram-isolation canary confirms ONLY telegram_client.py imports aiogram; B3 FIX precise attempt-tracker pattern verified by Tests 4/5/10/11/15 (cumulative 1/2/3 across success/retry/exhaustion + per-invocation isolation); 677 → 707 passed, 7 → 5 skipped; 3 Rule-1/3 auto-fixes inline (FK-on-DELETE-runs → probe nonexistent run_id=99999, aiogram token validator digits:string format, structural canary catches own docstring slip); SUMMARY: 06-03-SUMMARY.md]**
- [x] 06-04-PLAN.md - Wave 3: runners/delivery_run.py (run_delivery_phase 7-step + asyncio.run sync->async + Pitfall C path containment + Pitfall 6 single patch_stats + D-605/D-606/D-608) + cli.py AMEND (deliver-run subcommand + _cmd_deliver + load_dotenv only here per caveat #4) **[Complete 2026-05-12; 27 new GREEN tests across 2 closed Wave-0 stubs (19 delivery_run + 8 cli_deliver); D-605 invariant verified — TelegramNetworkError 3x leaves runs.status unchanged + xlsx on disk; D-606 6-enum reachable; D-607 Pitfall 6 single patch_stats — exactly 2 static call sites (1 in _build_stats_skip_path + 1 in main flow Step 7) enforced by structural canary; D-608 idempotency + --force + --dry-run; D-611 asymmetric ENV (TG_BOT_TOKEN required, business_chat_id degrades route, ops_chat_id missing on business route logs warning + proceeds); Pitfall C path containment defense-in-depth; Pitfall D message_id sentinels (non-split caption_id==document_id; split distinct); deliver-run CLI with 6 flags + load_dotenv ONLY in cli.py per RESEARCH caveat #4 (structural canary); 5 Rule-1/2/3 auto-fixes inline (Python 3.12 sqlite default datetime adapter → _coerce_started_at helper; structlog→caplog mismatch → switched Test 5c to capsys; orchestrator + CLI both emit JSON in dry-run → _extract_payload rfind; pending+dry_run → exit 0; WAVE3_CLOSURES canary update); 707 → 735 passed, 5 → 3 skipped; SUMMARY: 06-04-SUMMARY.md]**
- [x] 06-05-PLAN.md - Wave 4: main_run.py AMEND (5 amend points: imports + MainRunResult D-616 + pre-init + composition D-615 + 5 return-sites) + tests/integration/test_weekly_run_with_delivery.py E2E (SC#1 + SC#2 + D-605 invariant) + tests/test_delivery_source_lock.py structural canaries **[Complete 2026-05-12; runners/main_run.py amended +50 LOC (2 imports + 2 dataclass fields + 2 pre-init vars + composition gate + 5 return-site amendments with B4 deterministic D-616 invariant src.count('delivery_status=delivery_status')==5 AND src.count('delivery_route=delivery_route')==5); 6 GREEN E2E tests in test_weekly_run_with_delivery.py (SC#1 happy → delivered_business + business chat gets caption+xlsx; SC#2 viled-below-sanity → delivery NOT invoked; SC#2 canonical size_guard → delivered_ops_only + ops alert with run_id; D-605 TelegramNetworkError → runs.status=='success' + xlsx on disk + delivery_status=undelivered_telegram_unreachable; reporter-skipped cascade → delivery NOT invoked; 5-namespace integrity in DB row); 4 GREEN structural canaries in test_delivery_source_lock.py (D-514 inverted no-summary-builder/excel-builder/reporter-queries/reporter-archive substrings; delivery package shape 6 modules; aiogram-isolation only in telegram_client.py via line-by-line import scan with inline-comment stripping; load_dotenv NOT in delivery/* per RESEARCH caveat #4); Phase 6 stub-tracking story closed entirely (STUB_FILES=[]; WAVE4_CLOSURES regression list); 735 → 746 passed, 3 → 1 skipped (only remaining is pre-existing Phase 3 viled artificial-mutation, NOT Phase 6); 4 Rule-1/2/3 auto-fixes inline (log.info kwargs renamed status=/route= to avoid B4 canary collision; aiogram canary scoped to import lines with comments stripped; STUB_FILES narrowed 2→0 + WAVE4_CLOSURES; Test 6 fake_goldapple_phase explicit patch_stats mirroring Step 14); SUMMARY: 06-05-SUMMARY.md]**
...
| 6. Telegram Delivery + Ops/Business Split | 5/6 | In progress (Plans 06-01..06-05 shipped 2026-05-12; Wave 4 composition complete via Plan 06-05: runners/main_run.py amended with D-615 explicit composition gate `if r_result.status == "success" and r_result.xlsx_path:` AFTER reporter step + BEFORE Norm06+finalize, calling run_delivery_phase with engine+run_writer+repo_root+config+env; MainRunResult dataclass gains D-616 fields delivery_status: str = "pending" + delivery_route: str = "" populated on ALL 5 return-statement sites (viled-failed + goldapple-failed + matcher-failed + success + DATA-05-except) with B4 deterministic invariant; 6 GREEN E2E tests (SC#1 + SC#2 + D-605 + skip-cascade + 5-namespace integrity in DB row); 4 GREEN structural canaries (D-514 inverted source-lock + delivery package shape + aiogram-isolation + load_dotenv NOT in delivery/); Phase 6 stub-tracking story closed entirely (STUB_FILES=[]); 735 → 746 passed, 3 → 1 skipped; only Plan 06-06 doc cascade remains to formally close DELIVER-01..05 in REQUIREMENTS.md and propagate D-605/D-606/D-607/D-615/D-616 to STATE.md Accumulated Key Decisions) | - |
